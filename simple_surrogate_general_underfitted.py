...
params_rf:dict = {'n_estimators':np.arange(5,300,5).tolist(),
                ...
                'max_features':['log2'],
                 'max_depth':np.array([2,5,7,9,13,15]).tolist()}
...
# Perform a loop al around the different items for slicing the arrays
for idx, (dim,slice_arrs) in enumerate(slices.items()):
    
    # # Fit the models
    grid_s_rf_intrusion.fit(X_intrusion[:,slice_arrs],y_intrusion)
    ...
    grid_s_rf_sea.fit(X_sea[:,slice_arrs],y_sea)
    ...
    y_mixed = np.concatenate((y_intrusion.reshape((-1,1)),y_sea.reshape(-1,1)),axis=1).reshape((-1,2))
    grid_s_rf_dual.fit(X_sea[:,slice_arrs],y_mixed)

    print("Random Forest Estimator for SEA",
        grid_s_rf_dual.best_estimator_,
          grid_s_rf_dual.best_score_,
          grid_s_rf_dual.best_index_,
          grid_s_rf_dual.best_params_,
          grid_s_rf_dual.cv_results_)
    ...
    dump(obj=grid_s_rf_intrusion,
         file=f"RF_reg_intrusion_{dim}D.skops",
         )

    # Save the SEA estimator
    dump(obj=grid_s_rf_sea,
         file=f"RF_reg_sea_{dim}D.skops",
         )


    # # Save the dual estimator
    dump(obj=grid_s_rf_dual,
         file=f"RF_reg_dual_{dim}D.skops",
         )
