...
class Starbox_problem(ioh.problem.RealSingleObjective):
    ...
    def _function_computation(self,X_:np.ndarray)->None:
        if X_.shape[1] >= 5:
            X_1 = np.array([X_[0,0],X_[0,1],X_[0,2],X_[0,3],X_[0,4]]).reshape(1,-1)
        else:
            X_1 = X_.reshape(1,-1)


        if not isinstance(self.__regressor_model_sea,GridSearchCV):
            arr = np.array(self.__regressor_model_intrusion.predict(X=X_1)).ravel()

            self.__cur_intrusion = arr[0]
            self.__cur_sea = arr[1]
        
        else:
            self.__cur_intrusion:float = self.__regressor_model_intrusion.predict(X=X_1)[0]
            self.__cur_sea:float = self.__regressor_model_sea.predict(X=X_1)[0]
...
def modify_array(x_0:np.ndarray,dim:int):
    ...
    def decorator(func):
        def wrapper(x_inp:np.ndarray):
            ...
            if x_0.size == 5:
                if dim == 1:
                    modified_array = np.array([x_01[0],x_01[1],x_01[2],x_01[3],arr[0]]).ravel()
                elif dim == 3:
                    modified_array = np.array([arr[0],arr[1],x_01[2],x_01[3],arr[2]]).ravel()
                else:
                    modified_array = np.array(x_inp).ravel()  # No modification if type is unknown
            elif x_0.size > 5:

                if x_inp.size == x_0.size:
                    modified_array = arr
                else:
                    if dim == 1:
                        modified_array = np.hstack((np.array([x_01[0],x_01[1],x_01[2],x_01[3]]).reshape(1,-1),arr.reshape(1,-1))).ravel()
                    elif dim == 3:
                        modified_array = np.hstack((np.array([arr[0],arr[1],x_01[2],x_01[3],arr[2]]).reshape(1,-1),arr[3:].reshape((1,-1)))).ravel()
                    else:
                        modified_array = arr  # No modification if type is unknown

            #print(f"For dim: {dim}", "original array=", x_0, "forward to Problem:", modified_array)
...
def return_simulation_setup(initial_val:np.ndarray,seed:int= SEED,changing_dimensions:int=1,
    ...
    if not initial_val.size  >= 5:
        raise ValueError("The initial value should be of more than size 5")
...
def adjust_initial_input(x0:np.ndarray,dim:int):
    ...
    if not x0.size>=5:
        raise ValueError("The size of the array must be 5 or over")
    
    if x0.size == 5:
        if dim ==1:
            return np.array([x_01[4]]).ravel()
        elif dim ==3:
            return np.array([x_01[0], x_01[1],x_01[4]]).ravel()
        elif dim ==5:
            return x_01
        else:
        ...
        if dim == 1:
            varying_parameters = np.array([x_01[4]]).reshape((1,-1))
            return np.hstack((varying_parameters,x_01[5:].reshape(1,-1))).ravel()
        elif dim ==3:
            varying_parameters = np.array([x_01[0],x_01[1],x_01[4]]).reshape((1,-1))
            return np.hstack((varying_parameters,x_01[5:].reshape(1,-1))).ravel()
        elif dim ==5:
            return x_01
        else:
...
def main(pid_id,
    ...
    for idx,dim in enumerate(dimensions):
        ...
        logger_ioh = ioh.logger.Analyzer(
            ...
            algorithm_name=f"CMA-ES",    # meta-data for the algorithm used to generate these results
        ...
        if dim_case == 1:
            ...
            ff:ioh.problem.RealSingleObjective = Starbox_problem(model_dual,None,n_variables=n_vars,instance=1,
                                                                objective_type=obj_type,
                                                                lamda=lamda_val,opt_dimensionality=dim)
            ff2:ioh.problem.RealSingleObjective = Starbox_problem(model_dual,None,n_variables=n_vars,instance=2,
    ...
    for run in range(n_runs):
        ...
        if np.all(np.isnan(x0_)):
            x0 = np.random.uniform(low=-5,high=5,size=(1,n_vars))
        else:
            x0 = x0_.copy()
        ...
        for idx, dim in enumerate(dimensions):
            ...
            print(f"Adjusted input for dim={dim}:", x01 )

            if x0.size == 5:
                mu_0, lamda_0 = return_initial_mu_lamda(dim=x01.size,case=pop_case)
            else:
                # This design is bad
                mu_0, lamda_0 = return_initial_mu_lamda(dim=x01.size,case=0)
            ...
            if cma_case_handler == 1:
                ...
                opts = c_maes.parameters.Settings(dim=x01.size, modules=module_1,x0=x01, 
                                                sigma0 =sigma_0, budget=budget, verbose=verbose,
                                                mu0 = mu_0, lambda0 = lamda_0,
                                                lb=np.array([-5.0]*x01.size), 
                                                ub=np.array([5.0]*x01.size))
                
                opts2 = c_maes.parameters.Settings(dim=x01.size, modules=module_2,x0=x01, 
                                                sigma0 =sigma_0, budget=budget, verbose=verbose,
                                                mu0 = mu_0, lambda0 = lamda_0,
                                                lb=np.array([-5.0]*x01.size), 
                                                ub=np.array([5.0]*x01.size))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default= 5,
        type= int,
        help= "Set the total number of variables set for the optimizer",
    )


    args = parser.parse_args()

    if np.size(args.initial_point) != args.number_of_variables:
        args.initial_point = np.full(shape=(1,args.number_of_variables),fill_value=np.nan),
    ...
    main(pid_id=args.pid,
         ...
         x0_= np.array(args.initial_point).reshape((1,args.number_of_variables)),
         n_vars=args.number_of_variables,
         )